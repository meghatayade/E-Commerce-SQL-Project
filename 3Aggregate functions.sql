...
MariaDB [ecommerce_sales]> select count(*) as total_customers from customers;
+-----------------+
| total_customers |
+-----------------+
|              40 |
...
MariaDB [ecommerce_sales]> select count(*) as total_products from products;
+----------------+
| total_products |
+----------------+
|             25 |
...
MariaDB [ecommerce_sales]> select count(*) as total_orders from orders;
+--------------+
| total_orders |
+--------------+
|          180 |
...
MariaDB [ecommerce_sales]> select count(*) as total_orders from orders where order_status = "delivered";
+--------------+
| total_orders |
+--------------+
|          129 |
...
MariaDB [ecommerce_sales]> select sum(quantity) as total_quantity from orders ;
+----------------+
| total_quantity |
+----------------+
|            450 |
...
MariaDB [ecommerce_sales]> select sum(quantity * unit_price) as total_revenue from orders where order_status = "delivered" ;
+---------------+
| total_revenue |
+---------------+
|        297445 |
...
MariaDB [ecommerce_sales]> select avg(unit_price) as avg_product_price from products;
+-------------------+
| avg_product_price |
+-------------------+
|          939.0000 |
...
MariaDB [ecommerce_sales]> select round(avg(unit_price),2) as avg_product_price from products;
+-------------------+
| avg_product_price |
+-------------------+
|            939.00 |
...
MariaDB [ecommerce_sales]> select max(unit_price) as highest_price, min(unit_price) as lowest_price from products;
+---------------+--------------+
...
MariaDB [ecommerce_sales]> select avg(quantity) as avg_quantity_per_order from orders;
+------------------------+
...
MariaDB [ecommerce_sales]> select order_status, count(*) as total_orders from orders group by order_status;
+--------------+--------------+
...
MariaDB [ecommerce_sales]> select city, count(*) as total_customers from customers group by city;
+------------+-----------------+
...
MariaDB [ecommerce_sales]> select city, count(*) as total_customers from customers group by city order by total_customers desc;
+------------+-----------------+
| city       | total_customers |
+------------+-----------------+
| Kolkata    |               4 |
...
MariaDB [ecommerce_sales]> select avg(quantity * unit_price) as avg_order_value from orders where order_status = "delivered";
+-----------------+
| avg_order_value |
+-----------------+
|       2305.7752 |
...
MariaDB [ecommerce_sales]> select city, count(*) as total_customers from customers group by city having count(*) > 2;
+-----------+-----------------+
| city      | total_customers |
+-----------+-----------------+
| Bengaluru |               3 |
...
MariaDB [ecommerce_sales]> select payment_method, count(*) as total_orders from orders group by payment_method having count(*) > 20;
+------------------+--------------+
...
MariaDB [ecommerce_sales]> select category, avg(unit_price) as avg_product_price from products group by category having avg_product_price > 1000;
+-------------+-------------------+
| category    | avg_product_price |
+-------------+-------------------+
| Electronics |         1179.0000 |
| Fashion     |         1519.0000 |
+-------------+-------------------+
2 rows in set (0.064 sec)

MariaDB [ecommerce_sales]> select category, round(avg(unit_price),2) as avg_product_price from products group by category having avg_product_price > 1000;
+-------------+-------------------+
| category    | avg_product_price |
+-------------+-------------------+
| Electronics |           1179.00 |
